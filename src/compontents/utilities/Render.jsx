    ...
    return ( 
        <section className="h-screen relative text-white">
           <div id="background" className="h-screen relative pl-[60px] pt-[180px]">
                <h1 className='font-bold text-2xl md:text-4xl w-[60%]'>{defaultLoad.title || defaultLoad.name}</h1>

                <div id="rating">
                    <span>
                        Rating: {Math.floor(defaultLoad.vote_average)}/10
                    </span>
                </div>
            ...
            <div id="carousel" className="absolute bottom-0 left-0 flex h-[20vh] md:h-[17vh] text-xs overflow-y-hidden overflow-x-scroll w-screen px-[60px]">
                {
                    list.map(item => (
                        <button key={item.id} id={item.id} className="shrink-0 w-full md:w-[202px] relative" onClick={(e) => getDetails(e)}>

                            <h1 className='absolute z-[1] text-right w-[80%] bottom-6 right-6 font-bold'>{item.title || item.name}</h1>

                            <img src={img + item.backdrop_path} alt="" className="brightness-50 pointer-events-none object-cover w-full
                            h-full"/>
